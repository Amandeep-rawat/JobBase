import { deleteJobs, getSingleJob, updateHiringStatus } from '@/api/apiJobs'
...
import ApplyJobDrawer from '../Applyjobs'
...
const Job = () => {
  ...
  const handleStatusChange = (value) => {
    ...
    fnHiringStatus(isOpen)
      .then(() => {
        // console.log("Status updated, refetching job data"); // Debug log
        fnJob();
      });
  };
  ...
  return (
    <div className='flex flex-col gap-8 mt-5'>
      <div className='flex flex-col-reverse gap-6 md:flex-row justify-between items-center'>
        <h1 className='gradient-title font-extrabold pb-3 max-sm:text-3xl text-4xl sm:text-6xl'>{job?.title}</h1>
      ...
      {/* hiring status */}
      {loadingHiringStatus && <BarLoader width={"100%"} color="green"/>}
      {
